...
// POST /: Create task
router.post('/', authMiddleware, roleMiddleware(['ADMIN', 'HR']), validateOffboardingTask, async (req, res) => {
...
// PUT /:id: Update task
router.put('/:id', authMiddleware, roleMiddleware(['ADMIN', 'HR']), validateOffboardingTask, async (req, res) => {
  const { id } = req.params;
  const { employeeId, title, isCompleted } = req.body;
  try {
