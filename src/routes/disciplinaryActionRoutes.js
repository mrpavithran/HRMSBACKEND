...
// POST /: Create action
router.post('/', authMiddleware, roleMiddleware(['ADMIN', 'HR']), validateDisciplinaryAction, async (req, res) => {
...
router.put('/:id', authMiddleware, roleMiddleware(['ADMIN', 'HR']), validateDisciplinaryAction, async (req, res) => {
  const { id } = req.params;
  const { employeeId, type, reason } = req.body;
  try {
